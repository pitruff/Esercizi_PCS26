...
public:
	...
	//per l'implementazione di Dijkstra ho bisogno di un metodo in più, chenon mi restutisca solo un set con i vicini di un nodo ma una mappa che contenga anche i loro relativi pesi
	
	
	std::map<T, double> neighbours_with_weights(T nodo) const{
		
		std::map<T, double> risultato;
		
		for (const auto& e: archi){
			if (e.from() == nodo){
				risultato[e.to()]= e.weight();
			}
			else if (e.to() == nodo){
				risultato[e.from()]= e.weight();
			}
